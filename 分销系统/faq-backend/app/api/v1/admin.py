...
SUZHOU_SHEDE_WHITELIST: dict[str, dict[str, str]] = {
    ...
    "18114648396": {"name": "常恒浩", "role": "技术"},
    "19962915856": {"name": "韩钦宇", "role": "技术"},
    "15585390561": {"name": "杨飞侯", "role": "技术"},
    "18201833182": {"name": "杨晨曦", "role": "技术"},
    "17585602391": {"name": "张旭飞", "role": "技术"},
    "18862220827": {"name": "张化成", "role": "技术"},
    "18361938770": {"name": "李思锦", "role": "技术"},
    "19392051689": {"name": "付文强", "role": "技术"},
    "18862623670": {"name": "徐赞", "role": "技术"},
    "15952254821": {"name": "张宇晗", "role": "技术"},
    "18862603916": {"name": "屠皓天", "role": "技术"},
    "19352283603": {"name": "陈旭", "role": "技术"},
    "18994906575": {"name": "刘润泽", "role": "技术"},
    "13592256249": {"name": "罗富豪", "role": "技术"},
    "19052530116": {"name": "徐源鸿", "role": "技术"},
    "13205935339": {"name": "刘一尘", "role": "技术"},
    "15862403799": {"name": "袁孝威", "role": "技术"},
    "16607921837": {"name": "江乐群", "role": "技术"},
    "19209239140": {"name": "陈新瑞", "role": "技术"},
}
...
class ConferenceZoneCreatePayload(BaseModel):
    ...
    icon_url: Optional[str] = None
    sort_order: int = 0
    question_category: str = "conference_business"
...
class ConferenceQuestionCreatePayload(BaseModel):
    ...
    category: str = Field(
        ...,
        description="conference_business/conference_new_v/conference_digital/conference_channel",
    )
    is_active: bool = True
...
async def list_conference_zones(
    ...
    return {"code": 0, "data": [_zone_admin_dict(z) for z in zones.scalars().all()]}
...
async def create_conference_zone(
    ...
    if body.question_category not in CONFERENCE_QUESTION_CATEGORIES:
        raise HTTPException(status_code=400, detail="question_category 必须是 4 个大会题组之一")
    existing = await db.scalar(select(ConferenceZone.id).where(ConferenceZone.code == body.code))
    if existing:
        raise HTTPException(status_code=400, detail="打卡点 code 已存在")
    zone = ConferenceZone(
        code=body.code,
        name=body.name,
        slogan=body.slogan,
        ...
        active_from=_parse_iso_dt(body.active_from),
        active_to=_parse_iso_dt(body.active_to),
...
async def update_conference_zone(
    ...
    if (
        body.question_category is not None
        and body.question_category not in CONFERENCE_QUESTION_CATEGORIES
    ):
        raise HTTPException(status_code=400, detail="question_category 必须是 4 个大会题组之一")
    for key, value in body.model_dump(exclude_unset=True).items():
        if key in {"active_from", "active_to"}:
            setattr(zone, key, _parse_iso_dt(value))
        else:
...
async def list_conference_questions(
    ...
    stmt = select(Question).where(Question.category.in_(CONFERENCE_QUESTION_CATEGORIES))
    if category:
        if category not in CONFERENCE_QUESTION_CATEGORIES:
            raise HTTPException(status_code=400, detail="非大会题组")
        stmt = stmt.where(Question.category == category)
    stmt = stmt.order_by(Question.category, Question.id)
    questions = await db.execute(stmt)
    return {"code": 0, "data": [q.to_dict(hide_answer=False) for q in questions.scalars().all()]}
...
async def conference_dashboard(
    ...
    """每打卡点已发印章人数/参与率、题目完成率、已发勋章数。"""
    from app.services.conference import today_str
    ...
    attendees_count = int(
        await db.scalar(select(func.count(ConferenceAttendee.id))) or 0
    )
    medal_count = int(
    ...
    for zone in zones:
        ...
        answered_people = int(
            await db.scalar(
                select(func.count(func.distinct(ConferenceAnswer.phone))).where(
                    ConferenceAnswer.zone_id == zone.id
                )
            )
            or 0
        )
        zone_stats.append(
            {
                **_zone_admin_dict(zone),
                ...
                "participation_rate": (
                    round(stamp_count / attendees_count, 4) if attendees_count else 0.0
                ),
                "total_questions": total_questions,
                "answered_people": answered_people,
                "completed_people": stamp_count,
            }
        )
...
async def export_conference_zone_attendees(
    zone_id: int,
    db: AsyncSession = Depends(get_db),
    _: str = Depends(get_current_admin),
) -> StreamingResponse:
    ...
    marks_result = await db.execute(
        select(ConferenceZoneMark, ConferenceAttendee)
        .join(ConferenceAttendee, ConferenceAttendee.phone == ConferenceZoneMark.phone)
    ...
    xlsx_rows = [
        ...
        for mark, attendee in rows
    ]

    workbook = Workbook()
    sheet = workbook.active
    sheet.title = f"{zone.name}印章名单"
    _append_xlsx_rows(sheet, headers, xlsx_rows)
    ...
    workbook.save(stream)
    stream.seek(0)
    filename = f"{zone.name}-已领印章名单-{datetime.now():%Y%m%d}.xlsx"
    return StreamingResponse(
        stream,
        media_type="application/vnd.openxmlformats-officedocument.spreadsheetml.sheet",
        headers={"Content-Disposition": f"attachment; filename*=UTF-8''{quote(filename)}"},
